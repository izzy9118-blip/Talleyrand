...
def base_observation():
    return {
        ...
        "channel": {
            "origin": "Talleyrand", "immediate_sender": "Talleyrand", "intermediaries": [],
            "receiver": "Louis XVIII", "access_state": "direct documentary access", "formality": "official dispatch"
        },
...
def record():
    return {
        "record_type": "talleyrand_discovery_record", "protocol": "TAL-DISCOVERY-001",
        "inquiry_id": "INQ-TEST", "board_frozen_before_reading": True,
        "horus_source_selection_independent": True, "observations": [base_observation()],
...
def test_same_chain_transcription_does_not_cure_self_testimony():
    r = record(); o = r["observations"][0]
    o["support"] = [
        {"source_ref": "dispatch", "independence_group": "actor-account", "source_type": "SELF_TESTIMONY"},
        {"source_ref": "transcription", "independence_group": "actor-account", "source_type": "PRIMARY_TRANSCRIPTION"},
    ]
    with pytest.raises(DiscoveryProtocolError, match="self-testimony"):
        validate_discovery_record(r)
...
def test_documented_suppression_requires_mechanism_source():
    r = record(); r["observations"][0]["suppression"] = {"state": "DOCUMENTED", "mechanism_source_refs": []}
...
def test_search_failure_cannot_become_negative_evidence():
    r = record(); r["observations"][0]["source_absence"] = {"state": "SEARCHED_NOT_FOUND", "negative_claim": True}
    with pytest.raises(DiscoveryProtocolError, match="cannot assert absence"):
        validate_discovery_record(r)


def test_true_audience_claim_requires_ground():
    r = record(); r["observations"][0]["audience"]["true_audience_claim"] = True
    with pytest.raises(DiscoveryProtocolError, match="true-audience"):
        validate_discovery_record(r)


def test_specialist_claim_requires_competence_ground():
    r = record(); r["observations"][0]["technical_jurisdiction"] = {"requires_specialist": True, "competence_source_refs": []}
...
def test_documented_absence_requires_scope_and_positive_ground():
    r = record(); r["observations"][0]["source_absence"] = {"state": "DOCUMENTED_ABSENCE", "negative_claim": True, "scope": None}
