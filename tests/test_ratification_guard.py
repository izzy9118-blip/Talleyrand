...
def _template():
    return yaml.safe_load(Path("ratification/owner-decision.v11.template.yaml").read_text(encoding="utf-8"))


def _one_decision_progress():
    record = deepcopy(_template())
    record["id"] = "TAL-RAT-OWNER-DECISION-V11-TEST"
...
def _completed_all_ratify():
    record = deepcopy(_template())
    record["id"] = "TAL-RAT-OWNER-DECISION-V11-COMPLETE-TEST"
...
def test_live_dossier_is_v11_exact_and_nonoperative():
    result = validate_dossier(".")
    assert result["status"] == PASS_STATUS
    assert result["active_dossier"] == "TAL-RAT-DOSSIER-2026-08-09-011"
    assert result["pending_deed_decisions"] == 2
    assert result["prior_owner_decisions"] == 19
    assert result["live_ratified_deeds"] == 18


def test_active_dossier_contains_only_two_remaining_live_pending_deeds():
    dossier = yaml.safe_load(Path("ratification/2026-08-09-owner-review-dossier-v11.yaml").read_text())
...
def test_live_ratification_authority_carries_D2_and_exact_sharpening():
    ...
    assert len(record["deed_decisions"]) == 18
    decisions = {str(item["id"]): item for item in record["deed_decisions"]}
    assert decisions["D2"]["decision"] == "RATIFY"
    assert decisions["D2"]["git_blob_sha1"] == "9aac11883da3deb7d2d13d6177cbeaad1fac464a"
    for item in record["deed_decisions"]:
    ...
    d2 = bindings["TAL-DEED-D2-SHARP-001"]
    assert d2["deed"] == "D2"
    assert d2["git_blob_sha1"] == "2ec61eee9e9ea618b223035b8a5260a5d8442458"
    assert git_blob_sha1(Path(d2["path"])) == d2["git_blob_sha1"]
...
def test_carried_forward_owner_ratifications_include_D2_without_reopening_prior_deeds():
    dossier = yaml.safe_load(Path("ratification/2026-08-09-owner-review-dossier-v11.yaml").read_text())
    by_id = {str(x["id"]): x for x in dossier["prior_owner_decisions_not_reopened"]}
    assert set(by_id) == {"TAL-DISCOVERY-001", "0", "A1", "A2", "A3", "A4", "B1", "B2", "B3", "B4", "B6", "B7", "C1", "C3", "C4", "C7", "C8", "C11", "D2"}
    for item in by_id.values():
        assert item["decision"] == "RATIFY"
        assert git_blob_sha1(Path(item["path"])) == item["git_blob_sha1"]


def test_v11_template_has_only_two_live_pending_deeds_and_no_authority():
    result = validate_decision_record(_template(), ".", completed=False)
    assert result["completed"] is False
    assert result["deed_decisions"] == 2
    assert {str(x["id"]) for x in _template()["deed_decisions"]} == {"D3", "D4"}


def test_incremental_v11_record_can_decide_one_deed_without_inventing_others():
    ...
    assert record["deed_decisions"][0]["id"] == "D3"
...
def test_carried_forward_D2_text_change_invalidates_active_dossier(tmp_path):
    root = _copy_validation_surface(tmp_path)
...
def test_D2_sharpening_change_invalidates_active_dossier(tmp_path):
    root = _copy_validation_surface(tmp_path)
    path = root / "deeds/amendments/2026-08-09-D2-bounded-contingency-sharpening.md"
    path.write_text(path.read_text(encoding="utf-8") + "\nunauthorized drift\n", encoding="utf-8")
    with pytest.raises(RatificationGuardError, match="text changed after dossier freeze"):
        validate_dossier(root)


def test_dossier_cannot_pre_decide_D3(tmp_path):
    root = _copy_validation_surface(tmp_path)
    path = root / "ratification/2026-08-09-owner-review-dossier-v11.yaml"
